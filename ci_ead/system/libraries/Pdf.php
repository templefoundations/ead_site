<?php
/*
 * Changed the class name from FPDF to Pdf
 */
...
define('FPDF_FONTPATH',dirname(__FILE__).'/pdf_font/');
...
function Image_from_blob($file_obj, $x=null, $y=null, $w=0, $h=0, $link='')
{
	$file_name = $file_obj->get_name();
	//Put an image on the page
	if(!isset($this->images[$file_name]))
	{
		$type = $file_obj->get_image_type();
		//First use of this image, get info
		if($type=='')
		{
			$pos=strrpos($file_name,'.');
			if(!$pos)
				$this->Error('Image file has no extension and no type was specified: '.$file_name);
...
//		$info=$this->$mtd($file);
		$info = array(
			'w'=>$file_obj->get_image_width(),
			'h'=>$file_obj->get_image_height(),
			'cs'=>'DeviceRGB',
			'bpc'=>8,
			'f'=>'DCTDecode',
			'data'=>$file_obj->get_blob_data(),
		);
		$info['i']=count($this->images)+1;
		$this->images[$file_name]=$info;
	}
	else
		$info=$this->images[$file_name];
	//Automatic width and height calculation if needed
	if($w==0 && $h==0)
	{
		//Put image at 72 dpi
