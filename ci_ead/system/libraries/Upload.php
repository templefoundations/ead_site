...
class CI_Upload {
	...
	function validate($field = 'userfile', $upload=true)
	{
		...
		if ($upload &&  !$this->validate_upload_path())
		{
		...
		/*
		 * Set the image properties from the tmp file
		 */
	...
	function get_blob_data($field = 'userfile')
	{
		if ($this->validate($field, false)==true)
		{
			$fp = fopen($_FILES[$field]['tmp_name'],"r");
			$contents = fread($fp,$_FILES[$field]['size']);
			$data = addslashes($contents);
			fclose($fp);
			return $data;
			//			$data = @addslashes(fread(fopen($_FILES[$field]['tmp_name'],"r"),$_FILES[$field]['size']));
			//			return $data;
		}
		return false;
	}
